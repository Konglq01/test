  ...
  const onCreate = useCallback(
    async (values: any) => {
      try {
        ...
        if (state === 'register') {
          requestId = await requestRegisterDIDWallet({ managerAddress: _walletInfo.address });
        } else {
          requestId = await requestRecoveryDIDWallet({ managerAddress: _walletInfo.address });
        }
