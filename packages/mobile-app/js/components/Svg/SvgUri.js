...
class SvgUri extends Component {
  ...
  createSVGElement(node, child) {
    this.trimElementChildren(child);
    let componentAtts = {};
    const i = ind++;
    ...
    if (node.childNodes && node.childNodes.length > 0) {
      for (let i = 0; i < node.childNodes.length; i++) {
        ...
        if (isTextValue) {
          ...
          const node1 = this.inspectNode(node.childNodes[i]);
          if (node1 != null) {
            arrayElements.push(node1);
          }
        }
      }
    }
