...
import { importAccount, setCurrentAccount } from '@portkey-wallet/store/wallet/actions';
...
export default function ImportAccount() {
  ...
  const onInput = useCallback(async () => {
    if (!privateKey || !credentials?.password) return;
    setLoading(true);
    await sleep(500);
    try {
      dispatch(importAccount({ privateKey, password: credentials?.password }));
      ActionSheet.alert({
        ...
        ActionSheet.alert({
          ...
          buttons: [
            { title: i18n.t('Reimport'), type: 'outline' },
            {
              ...
              onPress: () => {
                dispatch(setCurrentAccount({ address: accountInfo.address, password: credentials.password }));
                navigationService.goBack();
              },
            },
          ],
    ...
    setLoading(false);
  }, [credentials?.password, dispatch, privateKey]);
