...
document.addEventListener('Portkey#initialized', (result) => {
  ...
  portkey.on('connect', (...args) => {
    console.log(args, 'connect== on');
  });

  portkey.on('chainChanged', (...args) => {
  ...
  portkey.on('accountsChanged', (...args) => {
    const loginInfo = document.getElementById('login-info');
    console.log(args, 'accountsChanged== on');
    loginInfo.innerHTML = JSON.stringify(args[0]);
  });
  portkey.on('onDisconnect', (...args) => {
  ...
  loginBtn.onclick = function () {
    aelf
      .login({
        ...
        rpcUrl: 'http://192.168.66.237:8000',
      })
      .then((result) => {
    ...
    tokenC.Transfer(
      {
        ...
        to: 'MiVc9oMt32Hm3EWtxvePzxZRhgYZT3ahfWJMBh1ZeVdRQ27Kw',
        amount: 1,
        memo: 'have fun',
      },
      (err, result) => {
