...
export default class PortKeyAelfProvider extends BaseProvider {
  ...
  private _listenerEvent = () => {
    window.portkey?.on('chainChanged', this._chainChanged);
    window.portkey?.on('lockStateChanged', this._lockStateChanged);
    // window.portkey?.on('accountsChanged', this._accountsChanged);
  };
