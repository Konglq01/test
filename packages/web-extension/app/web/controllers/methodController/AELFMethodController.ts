...
export default class AELFMethodController {
  ...
  getSignature = async (
    ...
    console.log(signatureInfo, 'getSignature===');
    const { hexToBeSign, account, appName, origin: _origin, appLogo = '' } = signatureInfo ?? {};
    if (!account) return sendResponse({ ...errorHandler(410002) });
    if (!hexToBeSign) return sendResponse({ ...errorHandler(400001, 'Missing param hexToBeSign.') });
    const connections: ConnectionsType = (await getLocalStorage('connections')) ?? {};
    const { permission } = connections[_origin] ?? {};
    const accountList = permission?.accountList ?? [];
    if (!accountList?.some((address) => account === address)) {
      return sendResponse({
        ...errorHandler(200016, `${account} is not connected to Portkey, please connect the user first`),
      });
    }
    const pageState = this._getPageState();
    const accountItem = pageState.wallet.accountList?.find((item) => item.address === account);
    if (!accountItem)
      return sendResponse({
        ...errorHandler(500001, 'Account error'),
      });
    const password = this._getPassword();

    const privateKeyData = await getPrivateKeyAndMnemonic(
      { AESEncryptPrivateKey: accountItem.AESEncryptPrivateKey },
      password ?? '',
    );

    if (!privateKeyData?.privateKey)
      return sendResponse({
        ...errorHandler(500001, 'Please unlock and try again'),
      });
    const privateAndUtils = AElf.wallet.ellipticEc.keyFromPrivate(privateKeyData.privateKey);
    const signedMsgObject = privateAndUtils.sign(hexToBeSign);
    const signature = [
      signedMsgObject.r.toString(16, 64),
      signedMsgObject.s.toString(16, 64),
      `0${signedMsgObject.recoveryParam.toString()}`,
    ].join('');
    const SignatureRes = await this.notificationService.openPrompt({
      method: PromptRouteTypes.GET_SIGNATURE,
      search: JSON.stringify({ appName, appLogo, appHref: _origin, signature, account, hexToBeSign }),
    });
    console.log(SignatureRes, 'SignatureRes==');
    sendResponse(SignatureRes);
  };
