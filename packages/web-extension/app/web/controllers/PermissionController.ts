...
export default class PermissionController {
  ...
  async checkIsLock(seed?: string | null, promptType: CreatePromptType = 'windows'): Promise<PortKeyResultType> {
    if (!seed) {
      return await this.notificationService.openPrompt(
        {
          method: InternalMessageTypes.PERMISSION_CONTROLLER,
        },
        promptType,
      );
    }
  ...
  async checkIsRegister() {
    ...
    if (registerStatus === 'registrationNotBackedUp' || registerStatus === 'Registered') {
      return true;
    } else {
      return false;
    }
  ...
  async checkIsRegisterOtherwiseRegister(method: string): Promise<PortKeyResultType> {
    ...
    if (registerStatus !== 'Registered') {
      return await this.notificationService.openPrompt(
        {
          method:
            InternalMessageTypes[registerStatus === 'registrationNotBackedUp' ? 'BACKUP_WALLET' : 'REGISTER_WALLET'],
        },
        'tabs',
      );
    } else {
